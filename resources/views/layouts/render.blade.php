...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .toolbar button,.toolbar a{display:inline-flex;align-items:center;gap:6px;height:38px;padding:0 16px;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;border:1px solid #e2e8f0;background:#fff;color:#334155;font-family:inherit;text-decoration:none}
        .toolbar button:hover,.toolbar a:hover{background:#f8fafc}
...
<body>
    @php
        $canDownloadPdf = auth()->check() && ((int) auth()->id() === (int) data_get($cv, 'user_id'));
    @endphp
    <div class="toolbar">
        <button type="button" onclick="window.location.href='{{ route('cvs.index') }}'">← Kembali</button>
        @if($canDownloadPdf)
            <a class="bp" href="{{ route('cvs.wizard.pdf', $cv) }}">Download PDF</a>
        @else
